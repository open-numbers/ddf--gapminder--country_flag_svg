...
def minify_b64_file(code):
    path = os.path.join('../source/b64', f'{code}.svg.txt')
    with open(path, 'r') as f:
        res = f.read()
    return res.replace('\n', '')
...
def main():
    ...
    country_df['flag_svg'] = country_df['iso3166_1_alpha2'].map(lambda x: minify_b64_file(x) if not pd.isnull(x) else x)
    ...
    geo_df['flag_svg'] = geo_df['iso3166_1_alpha2'].map(lambda x: minify_b64_file(x) if not pd.isnull(x) else x)
